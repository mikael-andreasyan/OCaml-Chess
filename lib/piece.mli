...
(**[valid_move (file_st, rank_st) (file_end, rank_end) piece] returns if a given
   ...
   only straight movements match the pattern, so diagonal movements need to be
   adjusted. This function is not able to check if a certain move is legal
   however (for example other pieces blocking the path or a piece not being able
   to move because it would put the king in check.). Note: The [file] is the
   letter associated with the column that the piece is on. For this function,
   the numbers 0-7 represent the letters a-h. Requires: [file_st, rank_st] and

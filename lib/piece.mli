...
(**[valid_move (file_st, rank_st) (file_end, rank_end) piece] returns if a given
   ...
   both diagonal and straight movements match the pattern. This function is not
   able to check if a certain move is legal however (for example other pieces
   ...
   that the piece is on. For this function, the numbers 0-7 represent the

(* *Quick array for piece values let pieceValuesMid = [| 82; 337; 365; 477;
   ...
   <<<<<<< HEAD (**[wkingShield board color] is a score for how protected the
   white king is. We basically want pawns in front of the king to help protect
   checkmate opporunities. *) let wkingShield board = let open Base.Int64 in let
   result = ref 0 in let pawns = ref (Board.get_piece_bitBoard board Board.pawn
   ...
   Stdlib.( = ) file 5 && Stdlib.( <= ) rank 4 then result := Stdlib.( + )
   !result shield2 else (); pawns := !pawns land (!pawns - one) done; !return)
   else 0 ======= (**[wkingShield board color] is a score for how protected the
   white king is. We basically want pawns in front of the king to help protect
   the king*) let wkingShield board = let open Base.Int64 in let result = ref 0
   in let pawns = ref (Board.get_piece_bitBoard board Board.pawn Board.white
   land shift_left Board.file1 1) in let king = Board.bit_to_tuple
   (Board.get_piece_bitBoard board Board.king Board.white) in if Stdlib.( = )
   (fst king) 0 && Stdlib.( > ) (snd king) 4 then ( let return = ref 0 in while
   !pawns <> zero do let lsb = ref (!pawns land neg !pawns) in let rank, file =
   Board.bit_to_tuple !lsb in if Stdlib.( = ) file 1 && Stdlib.( > ) rank 4 then
   ...
   (**[bkingShield board color] is a score for how protected the white king is.
   We basically want pawns in front of the king to help protect the king *) let
   bkingShield board = let open Base.Int64 in let result = ref 0 in let pawns =
   ref (Board.get_piece_bitBoard board Board.pawn Board.black land
   shift_right_logical Board.file8 1) in let king = Board.bit_to_tuple
   (Board.get_piece_bitBoard board Board.king Board.black) in if Stdlib.( = )
   (fst king) 0 && Stdlib.( > ) (snd king) 4 then ( let return = ref 0 in while
   !pawns <> zero do let lsb = ref (!pawns land neg !pawns) in let rank, file =
   Board.bit_to_tuple !lsb in if Stdlib.( = ) file 6 && Stdlib.( > ) rank 4 then
   result := Stdlib.( + ) !result shield1 else if Stdlib.( = ) file 5 &&
   Stdlib.( > ) rank 4 then result := Stdlib.( + ) !result shield2 else ();
   pawns := !pawns land (!pawns - one) done; !return) else if Stdlib.( = ) (fst
   king) 0 && Stdlib.( <= ) (snd king) 4 then ( let return = ref 0 in while
   !pawns <> zero do let lsb = ref (!pawns land neg !pawns) in let rank, file =
   Board.bit_to_tuple !lsb in if Stdlib.( = ) file 6 && Stdlib.( <= ) rank 4
   then result := Stdlib.( + ) !result shield1 else if Stdlib.( = ) file 5 &&
   Stdlib.( <= ) rank 4 then result := Stdlib.( + ) !result shield2 else ();
   pawns := !pawns land (!pawns - one) done; !return) else 0 >>>>>>>
   ...
   <<<<<<< HEAD (** [search_all_captures alpha beta board] searches through
   positions where only captures are available. *) let rec search_all_captures
   alpha beta board = let evaluation = eval board in if evaluation >= beta then
   beta else let alpha' = ref (max alpha evaluation) in let movesList =
   Board.legal_moves board in if Base.Array.is_empty movesList then if
   Board.player_check board (Board.current_turn board) then min_int else 0 else
   try for x = 0 to Base.Array.length movesList - 1 do let move = Base.Array.get
   movesList x in ignore (Board.make_move board move); let evaluation = -1 *
   ...
   -> !return ======= (** [search_all_captures alpha beta board] searches
   through positions where only captures are available. *) let rec
   search_all_captures alpha beta board = let evaluation = eval board in if
   evaluation >= beta then beta else let alpha' = ref (max alpha evaluation) in
   let movesList = Board.legal_moves board in try for x = 0 to Base.Array.length
   movesList - 1 do let move = Base.Array.get movesList x in ignore
   (Board.make_move board move); let evaluation = -1 * search_all_captures (-1 *
   beta) (-1 * !alpha') board in Board.unmake_move board; if evaluation >= beta
   then ( alpha' := beta; failwith "") else alpha' := max !alpha' evaluation
   done; !alpha' with _ -> !alpha'

   (** [search searchDepth alpha beta board] searches for the best move and
   returns the evaluation of the best move available. *) let rec search
   searchDepth alpha beta board = if searchDepth = 0 then search_all_captures
   alpha beta board else let movesList = Board.legal_moves board in let return =
   ref alpha in let alpha' = ref alpha in try for x = 0 to Base.Array.length
   movesList - 1 do let move = Base.Array.get movesList x in ignore
   (Board.make_move board move); let evaluation = -1 * search (searchDepth - 1)
   (-1 * beta) (-1 * !alpha') board in Board.unmake_move board; if evaluation >=
   beta then ( return := beta; failwith "") else alpha' := max !alpha'
   evaluation done; !return with _ -> !return >>>>>>> refs/remotes/origin/main
